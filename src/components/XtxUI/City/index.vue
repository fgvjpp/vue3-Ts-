<template>
  <div class="xtx-city">
    ...
    <div class="select" @click="toggleActive">
      <span class="placeholder" v-if="changeResult.provinceName === ''">请选择配送地址</span>
    ...
    <div class="option" v-if="activeFlag">
      <span class="ellipsis" v-for="i in chinaData" :key="i.code" @click="optionClick(i)">{{ i.name }}</span>
    </div>
  </div>
</template>
...
const changeResult = reactive<ChangeResult>({
  provinceCode: '', // 省code
  provinceName: '', // 省名称
  cityCode: '', // 城市code
  cityName: '', // 城市名称
...
function optionClick(cur: AreaList) {
  // 把当前点击的对象下的areaList赋值给负责渲染列表的数组
  if (cur.areaList) chinaData.value = cur.areaList
  ...
  (newValue) => {
    if (newValue === true) {
      for (const key in changeResult) {
        // 需要这样设置值
        Reflect.set(changeResult, key, '')
        // changeResult[key] = ''
      }
      // type aa = keyof ChangeResult
      // Object.keys(changeResult).forEach((key: aa) => {
      //   changeResult[key] = ''
      // })
